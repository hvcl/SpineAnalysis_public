function DendriticSpineImageProcessing_no_interpolation_210203(inputPath,resultPath)
    % path param
...
%     avestthreshB = mean(stthreshB(1:3));
    ...
    buffer=single(buffer)/single(max(buffer(:)));
    % interpolation _x4
%    [nY,nX,nZ]=meshgrid(1:ysize,1:xsize,1:0.032/0.12:zsize);
    %res1 = interp3(buffer,nX,nY,nZ,'linear');
    res1=buffer;
%    res1=imresize3(buffer,[ysize,xsize,round(zsize*zscaleFactor*LocalDoublingFactor)],interpolationMethod);
...
%    clear nX
    ...
    for k=1:Nsize(3)
        ...
        if k==1
            %buffer=Ithresh;
            buffer = activecontour(imagebuffer2, Ithresh, 50, 'edge','SmoothFactor',AcSmoothFactor,'ContractionBias',AcContractionBias);
        else
            %buffer(:,:,k)=Ithresh;
            buffer(:,:,k) = activecontour(imagebuffer2, Ithresh, 50, 'edge','SmoothFactor',AcSmoothFactor,'ContractionBias',AcContractionBias);
        end
    end
    res2=buffer;
    if zSmoothRange>1
...
%         res2=permute(res2,[3,2,1]);
    ...
    if LocalDoublingFactor>1
        res2=imresize3(single(res2),[Nsize(1),Nsize(2),round(Nsize(3)/LocalDoublingFactor)],interpolationMethod);
        res2=res2>zSmoothThresh;
        res1=imresize3(res1,[Nsize(1),Nsize(2),round(Nsize(3)/LocalDoublingFactor)],interpolationMethod);
    end
...
%     clear t
    ...
    [Istck2, Istck3A, Istck3B, Istck3C, Istck3D] = FitMainDendriteShaft(res2);
...
%     clear tres
    ...
    for k=1:MaxN 
        ...
        if VoxelNo > 20000
            ...
            if VoxelNoB > 2000 && (VoxelNo - VoxelNoB) > 392 
                Istck5 = Istck5-k*CheckMB; 
                dendrite=dendrite+CheckMB;        
            end
            if VoxelNoB < 2000 && VoxelNoC > 2000 && (VoxelNo - VoxelNoC) > 392 
                Istck5 = Istck5-k*CheckMC; 
                dendrite=dendrite+CheckMC;
            end
            if VoxelNoB < 2000 && VoxelNoC < 2000 &&VoxelNoD > 2000 && (VoxelNo - VoxelNoD)> 392 
...
%         CheckMD=imdilate(CheckMD,se1).*CheckM;
        ...
        if VoxelNoC>1.1 * VoxelNoD
            Istck5 = Istck5-k*CheckMC; 
    %        dendrite=dendrite+CheckMC;   
        elseif VoxelNoB>1.1 * VoxelNoC
            Istck5 = Istck5-k*CheckMB; 
    %        dendrite=dendrite+CheckMB;  
        end    
    ...
    clear CheckM
    clear CheckMA
    [Istck6, newK] = RefineSpineLocation(Istck5, MaxN);
...
%    IstckJunction = imdilate(IstckJunction, se5).*dendrite;
...
%     imwrite(labeloverlay(sum(res2,3)/max(max(sum(res2,3))),max(Istck6+tres,[],3)),[resultPath,'spineWithJunction.jpg']);
    ...
    for k = 1:newK 
        ...
        %% mesh simplification
        %[tri,v]=reducepatch(tri,v,simplificationFactor);
